...
namespace Tools.Core.Asserts
{
    ...
    public static class ErrorTrap
    {
        ...
        [SuppressMessage("Microsoft.Design", "CA1004:GenericMethodsShouldProvideTypeParameter",
            Justification = "T is type of an exception to be thrown. Parameter of type T can't be provided.")]
            ...
            )
        {
            ...
            if (messageId == null || !int.TryParse(messageId.ToString(), out resolvedMessageId))
            {
                Log.Source.TraceEvent(TraceEventType.Warning, 0,
                                      "messageId received in the call to AddAssertion is either null or can't " +
        ...
        public static void RaiseTrappedErrors<ExceptionType>() where ExceptionType : Exception, new()
        {
            ...
            var exception = Activator.CreateInstance(typeof (ExceptionType),
                                                     new object[] {"Conditions are not met: " + accumulatedText}) as
                            Exception;

            // If it is a base exception add accumulated messages as a collection
            var baseExceptionTest = exception as BaseException;
        ...
        [ThreadStatic] private static string accumulatedText;


        private static bool allowUIInteraction;
        [ThreadStatic] private static List<Message> messages = new List<Message>();
