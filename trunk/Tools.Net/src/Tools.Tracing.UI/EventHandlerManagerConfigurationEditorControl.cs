...
namespace Tools.Tracing.UI
{
	...
	/// </summary>
	public class EventHandlerManagerConfigurationEditorControl : System.Windows.Forms.UserControl
	{
		...
		private TraceEventHandlerManagerConfiguration _configuration = null;
		...
		public EventHandlerManagerConfigurationEditorControl()
		{
			...
			this.filterTreeView.AfterSelect += new TreeViewEventHandler(filterTreeView_AfterSelect);
		...
		private void buildTreeView()
		{
			...
			TreeNode root = new TreeNode(_configuration.Name);
		...
		{
			TreeNode root = new TreeNode("TypeSource");
			root.Tag = typeSource;
			
			TreeNode typeLocatorNode = new TreeNode("TypeLocator");
			...
			{
				TreeNode argNode = new TreeNode(a.Name);
				argNode.Tag = a;
		...
		private void loadFromFileMenuItem_Click(object sender, System.EventArgs e)
		{
			...
			if (openFileDialog1.FileName!=null||openFileDialog1.FileName!=String.Empty)
			{
				...
				{
					MessageBox.Show("Probably not a valid TraceEventHandlerManagerConfiguration Document! " + ex.ToString());
				}
			}
		}
		...
		private void copyToClipboardMenuItem_Click(object sender, System.EventArgs e)
		{
			System.Windows.Forms.Clipboard.SetDataObject
				(
				SerializationUtility.Serialize2String(this.Configuration),
				true
				);
		}

		private void loadFromClipboardMenuItem_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (System.Windows.Forms.Clipboard.GetDataObject().GetDataPresent(DataFormats.Text))
				{
					_configuration =
						(TraceEventHandlerManagerConfiguration)SerializationUtility.DeserializeFromString
						(
						Convert.ToString(System.Windows.Forms.Clipboard.GetDataObject().GetData(DataFormats.Text)),
						typeof(TraceEventHandlerManagerConfiguration)
						);
					// updateConfigurationGui();
					// temp placement
