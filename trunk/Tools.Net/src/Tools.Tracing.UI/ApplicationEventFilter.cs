...
namespace Tools.Tracing.UI
{
	...
	public class ApplicationEventFilter : IEnabled, IChangeEventRaiser
	{
		...
		public ApplicationEventFilter()
		{
			...
			_filterEntries.Changed += new System.EventHandler(this.filterEntryCollectionChanged);
		}
		...
		public bool Test(TraceEvent ae)
		{
			...
			for (int i = 0; i < this.FilterEntries.Count; i ++)
			{
				FilterEntry feCandidate = this.FilterEntries[i];
		...
		private string getMappedEventValue(string path, TraceEvent ae)
		{
			...
			//
			if (path=="TraceEvent::ContextIdentifier.ContextHolderId") return ae.ContextIdentifier.ContextHolderId.ToString();
			if (path=="TraceEvent::ContextIdentifier.ExternalId") return ae.ContextIdentifier.ExternalId.ToString();
			if (path=="TraceEvent::ContextIdentifier.ExternalParentId") return ae.ContextIdentifier.ExternalParentId.ToString();
			if (path=="TraceEvent::ContextIdentifier.ExternalReference") return ae.ContextIdentifier.ExternalReference.ToString();
			if (path=="TraceEvent::ContextIdentifier.InternalId") return ae.ContextIdentifier.InternalId.ToString();
			if (path=="TraceEvent::ContextIdentifier.InternalParentId") return ae.ContextIdentifier.InternalParentId.ToString();
			if (path=="TraceEvent::ContextIdentifier.AuthenticationTokenId") return ae.ContextIdentifier.AuthenticationTokenId.ToString();
