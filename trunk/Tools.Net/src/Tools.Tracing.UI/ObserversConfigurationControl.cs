...
namespace Tools.Tracing.UI
{
	...
	/// </summary>
	public class ObserversConfigurationControl : System.Windows.Forms.UserControl
	{
		...
		private System.Windows.Forms.ColumnHeader connectedColumnHeader;
		private System.Windows.Forms.MenuItem newRecordItem;
		private System.Windows.Forms.MenuItem deleteRecordMenuItem;
...
//		}
		...
		private void addConnectionRecord(RemoteConnectionConfiguration rcc, bool init)
		{
			...
			{
				EventsObserverInstance eoi =
					new EventsObserverInstance
                    (
                    rcc,
                    new TraceEventDelegate
                    (
                    _eventStub.HandleEvent
                    ));
				eoi.Changed += new EventHandler(rcc_Changed);
				//rcc.Changed += new EventHandler(rcc_Changed);
				if (!init) _connections.Add(rcc);
				if (!init) this.ConnectionInstances.Add(eoi);
...
//		}
		...
		private void newRecordItem_Click(object sender, System.EventArgs e)
		{
			initiateNewRecord();
		}

		private void deleteRecordMenuItem_Click(object sender, System.EventArgs e)
		{
			// TODO: Validate
			deleteConnectionRecord(editPropertyGrid.SelectedObject as RemoteConnectionConfiguration);
			// TODO: separate Gui thing from here
