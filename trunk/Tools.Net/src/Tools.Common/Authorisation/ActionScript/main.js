...
*/
var authorisationServiceUrl = "http://oghamweb/Vintavu.Host/TokenAuthorisationService.svc/VerifyToken?";
...
application.onConnect = function(p_client, p_autoSenseBW, p_clientSecurityKey)
{
	...
	keyParts  = p_clientSecurityKey.split(":");
	
	trace("Security key parsed into " + keyParts.length + " elements.");
	...
	try 
	{
		...
		var normToken = keyParts[1].searchReplace("+", "%2B")
		var authUrl = authorisationServiceUrl + "tokenTarget=" + keyParts[0] + "&token=" + normToken;
		
		trace("Calling authorisation service with url: " + authUrl);
...
AuthData.prototype.acceptAfterVerification = function(success)
{
	...
	{
		var keyParts  = new Array();	
		keyParts  = this._clientSecurityKey.split(":");
		
		trace("granted access to video folder (" + this._client.uri + "/" + keyParts[0] + ") to client with ip (" + this._client.ip + ")");
		
		
		this._client.readAccess = keyParts[0];
